...
class AVSUploader: NSObject, URLSessionTaskDelegate {
    ...
    fileprivate func parseResponse(_ data:Data, boundry:String) -> [PartData] {
        ...
        var innerRanges = [Swift.Range<Int>]()
        var lastStartingLocation = 0

        let startRange: Swift.Range = lastStartingLocation..<lastStartingLocation+data.count

        var boundryRange = data.range(of: innerBoundry, options: NSData.SearchOptions(), in: startRange)
        while(boundryRange != nil && boundryRange?.lowerBound != NSNotFound) {
            
            lastStartingLocation = (boundryRange?.upperBound)!
            var updatedRange: Swift.Range = lastStartingLocation..<data.count
            boundryRange = data.range(of: innerBoundry, options: NSData.SearchOptions(), in: updatedRange)
            
            if boundryRange != nil && boundryRange?.lowerBound != NSNotFound {
        ...
        for innerRange in innerRanges {
            ...
            let headerRange = innerData.range(of: "\r\n\r\n".data(using: String.Encoding.utf8)!, options: NSData.SearchOptions(), in: 0 ..< (innerRange.upperBound-innerRange.lowerBound))
            ...
            let contentData = innerData.subdata(in: startLocation! ..< (innerRange.upperBound-innerRange.lowerBound))
            
            let endContentRange = contentData.range(of: endBoundry, options: NSData.SearchOptions(), in: 0 ..< contentData.count)
            if endContentRange != nil && endContentRange?.lowerBound != NSNotFound {
    ...
    fileprivate func postRecording(_ authToken:String, jsonData:String, audioData:Data) {
        ...
        let uploadToAVSCb: (_ data:Data?, _ response:URLResponse?, _ error:Error?) -> Void = { (data:Data?, response:URLResponse?, error:Error?) -> Void in
        ...
        let uploadTask = self.session.uploadTask(with: request as URLRequest, from: bodyData as Data, completionHandler:  uploadToAVSCb)
