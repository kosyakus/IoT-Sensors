...
class SimplePCMRecorder {
    ...
    init(numberBuffers:Int) {
        self.recorderState = RecorderState(
            ...
            buffers: Array<AudioQueueBufferRef>(repeating: AudioQueueBufferRef.allocate(capacity: 0), count: numberBuffers),
            recordFile: nil,
            bufferByteSize: 0,
            currentPacket: 0,
            isRunning: false,
    ...
    func startRecording() throws {
        ...
        let cb : AudioQueueInputCallback = { (inUserData:UnsafeMutableRawPointer?, inAQ:AudioQueueRef, inBuffer:AudioQueueBufferRef, inStartTime:UnsafePointer<AudioTimeStamp>, inNumPackets:UInt32, inPacketDesc:UnsafePointer<AudioStreamPacketDescription>?) -> Void in

            let internalRSP = unsafeBitCast(inUserData, to: UnsafeMutablePointer<RecorderState>.self)
        ...
        let osAQNI = AudioQueueNewInput(&self.recorderState.dataFormat, cb, &self.recorderState, nil, nil, 0, self.recorderState.queue)
